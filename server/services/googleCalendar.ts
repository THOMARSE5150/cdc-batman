...
// Security Fix: Use environment variables for all credentials
...
// Get hostname dynamically from deployment environment
const getHostname = () => {
  ...
  // First priority: Railway production URL
  if (process.env.RAILWAY_STATIC_URL) {
    const url = `https://${process.env.RAILWAY_STATIC_URL}`;
    console.log('Using Railway Static URL:', url);
    return url;
  }
  
  // Second priority: Railway public domain
  if (process.env.RAILWAY_PUBLIC_DOMAIN) {
    const url = `https://${process.env.RAILWAY_PUBLIC_DOMAIN}`;
    console.log('Using Railway Public Domain:', url);
    return url;
  }
  
  // Third priority: General public URL (works for Railway, Vercel, Netlify)
  if (process.env.PUBLIC_URL) {
    console.log('Using PUBLIC_URL:', process.env.PUBLIC_URL);
    return process.env.PUBLIC_URL;
  }
  
  // Fallback to Replit for development/testing
  if (process.env.REPLIT_DOMAINS) {
    const url = `https://${process.env.REPLIT_DOMAINS}`;
    console.log('Using REPLIT_DOMAINS URL (development):', url);
    return url;
  }
  
  // Final fallback to localhost for local development
  console.log('Using localhost URL (local development)');
